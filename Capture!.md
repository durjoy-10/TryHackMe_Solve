...
![room_image](/Image/capture/i1.png)

- Here In this room A userlist and a password list has been given 
- Starting the machine, go to the login page and capture the login request to the burpsuite and understand it's functionality
- Functionality: After some attempts to login it gives a captcha (addition/subs/mul/div of two number). Solving this captcha i will then try to login..
- So make a python script by the Ai giving the request and response of this form ..And running this python script got the username and pass .
- Login via this username and pass and got the flag
...
- Run the script 
```
python3 capture.py http://ip/login usernames.txt passwords.txt 
```